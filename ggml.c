...
void ggml_print_tensor(struct ggml_tensor *tensor, int save_to_file) {
    ...
    char filename[256];
    sprintf(filename, "%s", tensor->name);
    replace_dots_with_underscores(filename);
    ...
    ggml_to_float_t to_float = type_traits[tensor->type].to_float;

    int ne3 = tensor->ne[3], nb3 = tensor->nb[3];
    int ne2 = tensor->ne[2], nb2 = tensor->nb[2];
